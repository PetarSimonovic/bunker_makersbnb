...
class BookingDetails
  ...
  def self.booking_details(user_id:)
    ...
    details = connection.exec("SELECT properties.property_id, properties.host_id, name, status, guest_id, booking_id FROM properties INNER JOIN bookings ON bookings.property_id = properties.property_id WHERE properties.host_id = '#{user_id.to_i}';")
